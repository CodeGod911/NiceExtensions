namespace NiceExtensions.Enumerable.Tasks
{
    public static partial class Tasks
    {
        ...
        /// <param name="throwAggException">throw an exception when one task fails</param>
        /// <param name="cancellationToken">The cancellation token.</param>
        public static async Task RunAndWhenAll(this IEnumerable<Task> tasksToRun, int maxActionsToRunInParallel = int.MaxValue, int timeoutInMilliseconds = -1, bool throwAggException = true, CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            var aggTask = Task.WhenAll(postTasks);

            if (throwAggException)
            {
                var exceptions = tasksToRun.Where(t => t.Exception != null).Select(t => t.Exception!).ToList();
        ...
        /// <param name="throwAggException">throw an exception when one task fails</param>
        /// <param name="cancellationToken">The cancellation token.</param>
        public static async Task<List<Task>> RunAndWhenAll(this IEnumerable<Func<Task>> tasksToRun, int maxActionsToRunInParallel = int.MaxValue, int timeoutInMilliseconds = -1, bool throwAggException = true, CancellationToken cancellationToken = new CancellationToken())
        {
            // Convert to a list of tasks so that we don't enumerate over it multiple times needlessly.
            using var throttler = new SemaphoreSlim(maxActionsToRunInParallel, maxActionsToRunInParallel);
        ...
        /// <param name="throwAggException">throw an exception when one task fails</param>
        /// <param name="cancellationToken">The cancellation token.</param>
        public static async Task<List<Task<TResult>>> RunAndWhenAll<TResult>(this IEnumerable<Func<Task<TResult>>> tasksToRun, int maxActionsToRunInParallel = int.MaxValue, int timeoutInMilliseconds = -1, bool throwAggException = true, CancellationToken cancellationToken = new CancellationToken())
        {
            // Convert to a list of tasks so that we don't enumerate over it multiple times needlessly.
            using var throttler = new SemaphoreSlim(maxActionsToRunInParallel, maxActionsToRunInParallel);
